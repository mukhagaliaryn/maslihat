...
const CommissionPlace = ({data}) => {
    const router = useRouter();
    const { t } = useTranslation();

    return (
        <Layout title={t("common:header.navbar.five.second")}>
            <div className="wrapper">
                <div className="title">
                    <h1>{t("common:header.navbar.five.second")}</h1>
                </div>
...
export async function getServerSideProps(context) {
    ...
    const res = await fetch(`${BACKEND_URL}/solution/category/4`)
...
export default CommissionPlace;